...
class UpdateButtonCell: NSButtonCell {
    ...
    var contentType: ContentType = .button {
        didSet {
            ...
            if contentType == .indeterminate {
                startDisplayLink(withDuration: nil)
            } else {
                displayLink = nil
            }
        }
    ...
    var updateProgress: Double = 0.0 {
        didSet {
            guard case .progress = contentType else {
                return
            }

            _oldUpdateProgress = oldValue
            startDisplayLink(withDuration: 0.2)
        }
    }

    private func startDisplayLink(withDuration duration: Double?) {
        displayLink = DisplayLink(duration: duration, callback: { [weak self] _ in
    ...
    override func highlight(_ flag: Bool, withFrame cellFrame: NSRect, in controlView: NSView) {
        ...
        view.animator().backgroundColor = (flag ? #colorLiteral(red: 0.7995074391, green: 0.8113409281, blue: 0.8403512836, alpha: 1) : #colorLiteral(red: 0.9488552213, green: 0.9487094283, blue: 0.9693081975, alpha: 1))
    }
    ...
    override func drawInterior(withFrame cellFrame: NSRect, in controlView: NSView) {
        ...
        case .none:
            return
        case .button:
            drawButtonBackground(withFrame: cellFrame)
        case .indeterminate:
            drawIndeterminateActivityIndicator(withFrame: cellFrame)
        case .progress:
            drawProgressIndicator(withFrame: cellFrame, controlView: controlView)
        }
    ...
    override func drawTitle(_ title: NSAttributedString, withFrame frame: NSRect, in controlView: NSView) -> NSRect {
        ...
        string.addAttribute(.font, value: NSFont.systemFont(ofSize: font!.pointSize - 1, weight: .medium), range: range)
    ...
    private func drawIndeterminateActivityIndicator(withFrame frame: NSRect) {
        guard var progress = displayLink?.progress else {
        ...
        indicatorColor(for: NSColor.tertiaryLabelColor).setStroke()
        path.stroke()
    }
    ...
    private func drawProgressIndicator(withFrame frame: NSRect, controlView: NSView) {
        ...
        // Draw pause blocks
        indicatorColor(for: Self.tintColor).set()
        renderPauseBar(offset: -2, from: center, in: controlView)
        renderPauseBar(offset: 2, from: center, in: controlView)

        var progress = updateProgress
        if let animationProgress = displayLink?.progress {
            progress = _oldUpdateProgress + ((updateProgress - _oldUpdateProgress) * animationProgress)
        }
    ...
    private func indicatorColor(for color: NSColor) -> NSColor {
        let tintColor: NSColor = (backgroundStyle == .emphasized ? .alternateSelectedControlTextColor : color)
        return isHighlighted ? tintColor.withSystemEffect(.pressed) : tintColor
    }
