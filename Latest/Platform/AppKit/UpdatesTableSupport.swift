...
final class SidebarTableMenuController: NSObject, NSMenuDelegate, NSMenuItemValidation {
  ...
  private func makeMenu() -> NSMenu {
    ...
    menu.addItem(
      menuItem(
        title: NSLocalizedString(
          "IgnoreAction", value: "Ignore", comment: "Action to ignore a given app."),
    ...
    menu.addItem(
      menuItem(
        title: NSLocalizedString(
          "UnignoreAction", value: "Don't Ignore", comment: "Action to stop ignoring a given app."),
...
final class SwiftUIUpdateTableView: NSTableView {
  ...
  override func menu(for event: NSEvent) -> NSMenu? {
    ...
    guard clickedRow >= 0,
      !(delegate?.tableView?(self, isGroupRow: clickedRow) ?? false)
    else {
      return nil
    }
