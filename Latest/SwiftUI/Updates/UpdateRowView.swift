...
struct UpdateRowView: View {
	...
	var body: some View {
		LegacyUpdateRowRepresentable(
			app: app,
			isSelected: isSelected,
			drawsSelectionBackground: false,
			filterQuery: filterQuery,
			dateFormatter: Self.dateFormatter,
			onSelect: { viewModel.select(app) }
		)
	...
	private var rowContextMenu: some View {
		...
		Button(NSLocalizedString("OpenAction", comment: "Action to open a given app.")) {
			viewModel.open(app)
		}

		Button(NSLocalizedString("RevealAction", comment: "Reveal in Finder Row action")) {
			viewModel.revealInFinder(app)
		}

		if app.isIgnored {
			Button(NSLocalizedString("UnignoreAction", value: "Don't Ignore", comment: "Action to stop ignoring a given app.")) {
				viewModel.setIgnored(false, for: app)
			}
		} else {
			Button(NSLocalizedString("IgnoreAction", value: "Ignore", comment: "Action to ignore a given app.")) {
...
private struct LegacyUpdateRowRepresentable: NSViewRepresentable {
	let app: App
	let isSelected: Bool
	...
	func makeNSView(context: Context) -> LegacyUpdateRowContentView {
		let view = LegacyUpdateRowContentView()
		view.onSelect = onSelect
		view.update(app: app, isSelected: isSelected, drawsSelectionBackground: drawsSelectionBackground, filterQuery: filterQuery, dateFormatter: dateFormatter)
		return view
	}

	func updateNSView(_ view: LegacyUpdateRowContentView, context: Context) {
		view.onSelect = onSelect
		view.update(app: app, isSelected: isSelected, drawsSelectionBackground: drawsSelectionBackground, filterQuery: filterQuery, dateFormatter: dateFormatter)
	}
}
