...
struct AppListSettings: Observable {
    ...
    enum SortOptions: Int, CaseIterable {
        ...
        var displayName: String {
            switch self {
            case .updateDate:
                NSLocalizedString("DateSortOption", comment: "Update date sorting option. Displayed in menu with title: 'Sort By' -> 'Date'")
            case .name:
                NSLocalizedString("NameSortOption", comment: "Sorting option to list by app names alphabetically. Displayed in menu with title: 'Sort By' -> 'Name'")
            }
        }
    }
    ...
    static var shared: AppListSettings = .init()
