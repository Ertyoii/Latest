...
class ReleaseNotesProvider {
    ...
    func releaseNotes(for app: App, with completion: @escaping (ReleaseNotes) -> Void) {
        ...
        if let releaseNotes = cache.object(forKey: app) {
            completion(.success(releaseNotes))
            return
        }

        loadReleaseNotes(for: app) { releaseNotes in
            if case let .success(text) = releaseNotes {
    ...
    private func loadReleaseNotes(for app: App, with completion: @escaping (ReleaseNotes) -> Void) {
        if let releaseNotes = app.releaseNotes {
            ...
            case let .html(html):
                Task {
                    await completion(self.releaseNotes(from: html, baseURL: nil))
                }
            case let .url(url):
    ...
    private func releaseNotes(from url: URL) -> AsyncThrowingStream<ReleaseNotesContent, Error> {
        if let github = GitHubReleaseRequest.from(url: url) {
            return releaseNotes(fromGitHub: github)
        }
        ...
        AsyncThrowingStream { continuation in
            Task { @MainActor in
                do {
                    ...
                    let header: String = switch (title, tag) {
                    case (nil, nil):
                        ""
                    case (let t?, nil):
                        t
                    ...
                    let linkHTML = if let url = release.htmlURL?.absoluteString {
    ...
    private func releaseNotes(from html: String, baseURL: URL?) async -> ReleaseNotes {
        ...
        let normalizedHTML = await Task.detached(priority: .userInitiated) {
            Self.normalizeHTMLForAttributedString(html)
        }.value
...
private extension ReleaseNotesProvider {
    ...
    nonisolated static func normalizeGitHubBodyHTML(_ html: String) -> String {
        ...
        normalizeHTMLForAttributedString(html)
    }
}
