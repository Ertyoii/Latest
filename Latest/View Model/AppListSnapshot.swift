...
struct AppListSnapshot {
    ...
    init(withApps apps: [App], filterQuery: String?) {
        ...
        self.apps = apps
        entries = Self.generateEntries(from: apps, filterQuery: filterQuery)
    }

    /// Returns a new snapshot containing an updated filter query.
    func updated(with filterQuery: String?) -> AppListSnapshot {
        AppListSnapshot(withApps: apps, filterQuery: filterQuery)
    }
    ...
    private static func generateEntries(from apps: [App], filterQuery: String?) -> [Entry] {
        ...
        visibleApps = visibleApps.filter { app in
            // Apply filter query
            if let filterQuery, !app.name.localizedCaseInsensitiveContains(filterQuery) {
                return false
            }
        ...
        var ignoredUpdates = filteredApps.filter(\.isIgnored).map { Entry.app($0) }
    ...
    func app(at index: Int) -> App? {
        if case let .app(app) = entries[index] {
            return app
        }
...
extension AppListSnapshot {
    ...
    enum Entry: Equatable, Hashable {
        ...
        func isSimilar(to entry: Entry) -> Bool {
            switch self {
            case let .app(app):
                if case let .app(other) = entry {
                    return app.identifier == other.identifier
                }
            case let .section(section):
                if case let .section(other) = entry {
    ...
    struct Section: Equatable, Hashable {
        ...
        static func == (lhs: Section, rhs: Section) -> Bool {
            lhs.title == rhs.title && lhs.numberOfApps == rhs.numberOfApps
        }
