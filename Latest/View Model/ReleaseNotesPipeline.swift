...
struct ReleaseNotesCandidateScorer: Sendable {
	...
	func quality(
		...
		if !candidate.declaredPlatforms.isEmpty {
			...
			guard candidate.declaredPlatforms.contains(where: {
...
struct ReleaseNotesResolver: Sendable {
	func resolve(
		...
		for candidate in candidates {
			do {
				accepted.append(ScoredReleaseNotesCandidate(
					candidate: candidate,
...
struct ReleaseNotesFetcher: Sendable {
	...
	func fetchMarkup(from url: URL, accept: String = "text/html,application/xhtml+xml,application/xml;q=0.9,text/plain;q=0.9") async throws -> String {
		...
		let declaredEncoding = result.response.textEncodingName.flatMap(String.Encoding.ianaCharacterSetName)
		let text = declaredEncoding.flatMap { String(data: result.data, encoding: $0) } ??
			String(data: result.data, encoding: .utf8) ??
			String(data: result.data, encoding: .isoLatin1)
		guard let text,
		      !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
		      !ReleaseNotesMarkup.looksLikeBinaryOrMojibakeText(text) else {
