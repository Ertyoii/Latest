...
final class AppLibrary: @unchecked Sendable {
    ...
    var bundles: [App.Bundle] {
        directories.flatMap(\.value.bundles)
    }
    ...
    private func setupDirectoryObservers() {
        ...
        var dispatchGroup: DispatchGroup? = directories.isEmpty ? DispatchGroup() : nil
    ...
    private lazy var directoryStore = AppDirectoryStore(updateHandler: self.startQuery)
}
