...
struct Version: Hashable, Comparable {
    ...
    var isEmpty: Bool {
        let versionNumberComponents = versionNumber?.components().compactMap(\.plainComponent).joined()
        let buildNumberComponents = buildNumber?.components().compactMap(\.plainComponent).joined()
    ...
    private static func compare(_ lhs: Version, _ rhs: Version) -> CheckingResult {
        ...
        for i in 0 ..< min(count1, count2) {
            guard case let .component(component1) = c1[i], case let .component(component2) = c2[i] else { continue }
            ...
            for i in 0 ..< min(atomsCount1, atomsCount2) {
                ...
                // Compare numbers
                if case let .number(value1) = component1, case let .number(value2) = component2 {
                    if value1 > value2 {
                ...
                // Compare letters
                else if case let .string(value1) = component1, case let .string(value2) = component2 {
        ...
        if count1 != count2 {
            ...
            guard case let .component(atoms) = longerComponents.first(where: { if case .component = $0 { true } else { false } }) else {
                return .equal // Think "1.2" vs "1.2."
            }

            if case let .number(number) = atoms.first {
                if number == 0 {
...
extension Version: CustomDebugStringConvertible {
    var debugDescription: String {
        "Version: \(versionNumber ?? "None"), Build: \(buildNumber ?? "None")"
    }
}
...
private extension Version {
    enum Segment: Equatable {
        ...
        var plainComponent: String? {
            guard case let .component(atoms) = self else {
                return nil
            }
...
extension [Version.Segment] {
    func joined() -> String? {
        let string = map { segment in
            switch segment {
...
extension OperatingSystemVersion {
    init(string: String) throws {
        let components: [Int] = string.components().flatMap { component in
            switch component {
            case let .component(atoms):
