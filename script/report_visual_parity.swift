#!/usr/bin/env swift  //  Fork contributions © 2026 ertyoii. First committed in this fork 2026-08-29.  //  Licensed under GPL-3.0; see LICENSE.md.
import AppKit
import Foundation
...
do {
  ...
  guard arguments.count == 3 || (arguments.count == 4 && arguments[3] == "--require-exact") else {
    throw ParityError.invalidArguments
  }
  ...
  for reference in references {
    ...
    guard FileManager.default.fileExists(atPath: actual.path) else {
      throw ParityError.missingActual(actual)
    }
  ...
  print(
    "Compared \(comparedImages) image pairs; \(exactImages) were byte-for-byte pixel identical after RGBA decoding."
  )
