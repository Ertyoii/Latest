...
class UpdateInstaller: NSObject, UpdateInstallerProtocol {
  func performInstallation(
    ...
    do {
      // Install app
      let path = url.path()
      let (success, output) = try performCommand(
        "/usr/sbin/installer -pkg '\(path)' -target '\(targetURL)'")
      guard success else {
        reply(
          NSError(
            domain: "LatestInstallerErrorDomain", code: 0,
      ...
      try fileManager.setAttributes(
        attributes, ofItemAtPath: receiptURL.path(percentEncoded: false))
    } catch {
      reply(error)
      return
    }
