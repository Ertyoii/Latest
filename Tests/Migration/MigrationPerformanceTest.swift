...
final class MigrationPerformanceTest: XCTestCase {
	func testMigrationPerformanceMatrix() throws {
		...
		let scrollSnapshot = AppListSnapshot(withApps: try XCTUnwrap(appsBySize[1_500]), filterQuery: nil)
		...
		let detailViewModel = UpdatesListViewModel(snapshot: AppListSnapshot(withApps: detailApps, filterQuery: nil))
		...
		let detailHost = NSHostingView(rootView: ReleaseNotesDetailView(
	...
	private func emitBenchmarkLine(
		...
		let line = String(
			format: "MIGRATION_BENCHMARK name=%@ iterations=%d min_ms=%.3f avg_ms=%.3f p50_ms=%.3f p95_ms=%.3f max_ms=%.3f checksum=%d",
	...
	private func emitMemoryLine(name: String, before: UInt64, after: UInt64) {
		...
		let line = "MIGRATION_MEMORY name=\(name) before_bytes=\(before) after_bytes=\(after) delta_bytes=\(delta)"
		FileHandle.standardError.write(Data((line + "\n").utf8))
	}
	...
	private func makeApps(count: Int) -> [Latest.App] {
		(0..<count).map { index in
			...
			let update = Latest.App.Update(
				...
				releaseNotes: .html(string: "<h2>Version 2.\(index)</h2><p>Migration fixture notes for app \(index).</p>"),
